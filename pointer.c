...
void homework(int *stress, int *happy, int *hp) {
	*stress += 20; //homework 수행 시, stress 20 증가.
	*happy += 10; //과제를 끝내서 뿌듯하므로 happy 10 증가.
	*hp -= 10; //과제를 하는 동안 체력 10 감소.
	printf("과제를 진행합니다.\n");
	Sleep(1500);
...
void play(int *stress, int *hp, int *money) {
	*stress -= 30; // 친구와 놀 때 스트레스 30 감소.
	*hp -= 30; //체력 30 감소
	*money -= 30000; //30000원 소비
	printf("신나게 친구들과 술을 마십니다.\n");
	Sleep(1500);
	printf("스트레스는 확 줄었으나, 돈과 체력을 잃었습니다.\n");
}
void eat(int *happy, int *hp, int *money) {
	*happy += 15; // 밥 먹으면 행복하니까 행복 15 증가.
	*hp += 20; // 에너지도 충전되니까 체력 20 증가.
	*money -= 8000; //식비는 자기가 부담하므로 8000원 감소.
...
void sleep(int *stress, int *hp, int time) {
	if (time >= 6) {
		...
		Sleep(500 * time);
		*stress -= 20; // sleep 수행 시, 6시간 이상이면 stress 20 감소
		*hp += 30; // 체력 30 증가.
		...
		printf("숙면을 취합니다. 수면시간이 길 수록 대기시간도 길어집니다.\n");
		Sleep(500 * time);
		*stress -= 10; // 6시간 미만이면 stress 10 감소.
		*hp += 15; // 체력 15 증가
...
void alba(int *stress, int *hp, int *money, int time) {
	...
	Sleep(500*time);
	*stress += 5 * time; //알바하는 것만큼 싫은 게 없다. 스트레스 시간 당 5 증가.
	*hp -= 10 * time;; //알바하는 것만큼 힘든 것도 없다. 체력 시간 당 10 감소.
	*money += 8350 * time; //최저시급도 감지덕지.
...
void show_menu(int stress, int happy, int hp, int money, int date) {
	...
	if (happy >= 100 || stress <= -150) printf("**요즘 행복해서 웃으면서 지내고 계시군요?**\n");
	if (happy <= -50 || stress >= 50) printf("**아무리 일이 중요해도 행복이 중요하지 않을까요?**\n");
	if (money > 250000) printf("**이 정도 돈이면 좀 놀고 먹어도 되겠는걸요?**\n");
	if (hp <= 0) printf("**체력이 낮아도 행동은 가능하나, 건강을 생각해주세요!**\n");
	if (money <= 10000) printf("**돈이 적어도 행동은 가능하나, 신용등급을 생각해주세요!**\n");
